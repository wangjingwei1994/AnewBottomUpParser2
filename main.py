...
def run_train(args):
    ...
    dev_parse = [tree.convert() for tree in train_treebank]
    ...
    def check_dev():
        ...
        for tree in dev_parse:
            ...
            predicted, _, _, good1, bad1, good2, bad2, positive1, positive2, positive1_good, positive2_good  = parser.parse(sentence, False, level_labels, connect_tree, tree, args.explore)
            good_label+=good1
            good_con+=good2
            bad_label+=bad1
            bad_con+=bad2
            positive_label += positive1
            positive_con += positive2
            good_positive_label += positive1_good
            good_positive_con += positive2_good
            if args.parser_type == "bottom-up":
        ...
        print(
            ...
            "total-elapsed {}".format(
                good_label/(good_label+bad_label),
                good_positive_label / good_label,
                good_positive_label / positive_label,
                good_con/(good_con+bad_con),
                good_positive_con / good_con,
                good_positive_con / positive_con,
                dev_fscore,
        ...
        for start_index in range(0, len(train_parse), args.batch_size):
            ...
            for tree in train_parse[start_index:start_index + args.batch_size]:
                ...
                if args.parser_type == "bottom-up":
                    ...
                    level_tree, max_level, star = convert_to_level_trees(words, tags, tree)
                    connect_tree, level_labels = get_connect_tree(words, level_tree, max_level, star)
                    _, loss_label, loss_connect,  good1, bad1, good2, bad2, positive1, positive2, positive1_good, positive2_good = parser.parse(sentence, True, level_labels, connect_tree, tree, args.explore)
                    good_label +=good1
                    good_con+=good2
                    bad_label+=bad1
                    bad_con+=bad2
...
def main():
    ...
    class item:
        def __init__(self):
            ...
            self.batch_size = 10
            self.epochs = 4
            self.checks_per_epoch = 10
            self.print_vocabs = True
            #self.model_path_base = "/home/wang/minimal-span-parser-master/model"
    args = item()
    run_train(args)
    ...
    class item2:
        def __init__(self):
            ...
            self.model_path_base = "E:\Anew\model_dev=22.66"
    args2 = item2()
    run_test(args2)
