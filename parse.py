...
class BottomUpParser(object):
    ...
    def parse(self, sentence, is_train, level_gold=None, connect_tree=None, gold=None, explore=True):
        #is_train = gold is not None
        good1 = 0
        bad1 = 0
        good2 = 0
        ...
        for i in range(len(lstm_scores_labels)):
            if is_train:
                ...
                oracle_label_index = self.label_vocab.index(oracle_label)
                looklook = lstm_scores_labels[i].npvalue()
                looklook2 = lstm_scores_connect[i].npvalue()
                lstm_scores_labels[i] = augment(lstm_scores_labels[i], oracle_label_index)
                label_scores_np = dy.softmax(lstm_scores_labels[i]).npvalue()
        ...
        for i in range(len(lstm_scores_connect)):
            if is_train:
                ...
                connect_scores_np = dy.softmax(lstm_scores_connect[i]).npvalue()
                ...
                if oracle_connect == 1:
                    positive2 +=1
                if argmax_connect == oracle_connect:
                    good2 +=1
                    if argmax_connect == 1:
                        positive2_good +=1
                else:
        ...
        def Helper(next_labels, temp_connect, next_connect, good1, bad1, good2, bad2, positive1, positive2, positive1_good, positive2_good):
            lstm_scores_labels = []
            lstm_scores_connect = []
            span = []
            i=0
            ...
            return level_label, loss_i_label, level_connect, loss_i_connect, good1, bad1, good2, bad2, positive1, positive2, positive1_good, positive2_good



        connect = first_level_connect[:]
        level = 0
        while (gold is not None and level+1 < len(connect_tree)) or (gold is None and len(connect) != 1) and (level <= len(sentence)):
            if gold is not None:
                ...
                temp_labels = []
                next_connect = []
            label, lossi_label, connect, lossi_connect, good1, bad1, good2, bad2, positive1, positive2, positive1_good, positive2_good = \
                Helper(temp_labels, temp_connect, next_connect, good1, bad1, good2, bad2, positive1, positive2, positive1_good, positive2_good)
        ...
        return tree_str,label_loss,connect_loss, good1, bad1, good2, bad2, positive1, positive2, positive1_good, positive2_good
